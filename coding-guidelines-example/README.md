...
| 20 | What flight software does | `orbit/Kepler.cpp` | The Newton loop is bounded (rule 2) **and reports non-convergence** (rule 5) — the half most numerical code omits. Assertions on the impossible (rule 5). One allocation, up front (rule 3's spirit). Short functions (rule 4). |
