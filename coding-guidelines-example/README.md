...
|---|---|---|
| Builds under clang's `-Weverything`, **as errors** | `cmake --build build` | zero warnings |
| Same, with assertions live | `cmake --build build-debug` | zero warnings |
| Builds under gcc-14 with every warning it lists, **as errors**, Debug and RelWithDebInfo | `cmake -S . -B build-gcc -DCMAKE_CXX_COMPILER=g++-14`, under WSL | zero warnings, the same 47 checks |
...
| Every header compiles standalone (SF.11) | `orbex_header_selfcheck` target | 7 / 7 |
...
The warnings are the parent project's policy
([its ADR 0017](../docs/adr/0017-every-warning-is-an-error.md)): clang's
`-Weverything` less three exceptions written beside their flags, and for gcc
`cmake/GccWarnings.cmake`, every warning gcc-14 lists for C++, generated by
the parent's `scripts/gcc-warnings.py`. It is a copy, so this directory still
builds on its own; re-generate it when gcc is upgraded.
...
|---|---|---|---|
| 1 | Use the tools available | `CMakeLists.txt` | Every warning the compiler has — clang's `-Weverything`, gcc's full list — carried on an INTERFACE target with `-Werror`, so no target can quietly opt out, and each exception written beside its flag with its reason. Plus a sanitizer option, a `.clang-tidy` at `WarningsAsErrors: '*'`, and a generated TU per header. |
| 2 | Express intent, hard-to-misuse interfaces | `core/Units.hpp`, `orbit/Kepler.hpp` | Six strong types — aliases of one `Scalar<kReference>` template over mp-units — every constructor `explicit`. `solveKepler(Radians, Eccentricity)` cannot be called transposed and a `static_assert` proves it. `Vec3<kReference>` carries its unit too, so a position and a velocity do not add, and `cross(r, v)` has a unit nothing had to name. `PathOptions` collapses three parameters into one (I.23). `PathClosure`/`Spacing` instead of bools. Preconditions stated in the doc comment and checked in code. |
...
| 11 | Floating point | `render/PathUpload.cpp`, `CMakeLists.txt`, `core/Units.hpp` | `-ffp-contract=off`, never `-ffast-math`. **No `==` on floats anywhere**: every strong type deletes the one its defaulted `<=>` would bring, `-Wfloat-equal` reports one on a bare double, and a `static_assert` proves the types have none. **Three `static_cast<f32>`, all in one function**, and they subtract in f64 first. NaN-safe negated preconditions. |
...
| 16 | Comments | `orbit/Kepler.cpp` | Every constant says where its number came from — including the one where the honest answer is *nobody knows*: `kHighEccentricity` is 0.8 because the heuristic is widespread, and the claim that it was "Danby's threshold" was withdrawn on 2026-09-13 when it was checked (Danby's 0.85 is a coefficient on e, not a threshold on e). `kConvergenceUlps` is 4.0, in units of the last place rather than the absolute 1e-14 it replaced, and the comment says why that mattered. `kMaxIterations` is 100 and is not a performance budget. And the eccentric-anomaly comment, which exists to stop a future reader "simplifying" it. *(This row said "0.8 is Danby's threshold, 1e-14 …, 50 …" until 2026-09-13: three constants named, three numbers wrong.)* |
| 17 | Maintainability | `docs/adr/`, `.clang-tidy` | Three ADRs for the decisions that span files — 0003 replaced six copies of the same eight lines with one template and six aliases, which is this rule applied to the example itself. Longest function is `OrbitPath::sample`; `readability-function-size` enforces the limit. `Velocity`/`Mass` deliberately unwritten. Every lint suppression carries its reason. |
...
| 19 | Determinism | `tests/test_orbit_path.cpp`, `tests/test_kepler.cpp`, `core/Vec3.hpp` | Pure functions by construction. The determinism test asserts bit-identical output across two runs, through `bitIdentical()`, which compares bits and so tells +0.0 from -0.0. The randomised test seeds `mt19937_64` with a fixed, written-down value. |
...
## Three places the rules collide
...
scenario exactly.** This example used to answer that with `==` in
`testDeterminism`, on the argument that when the claim is bit identity, `==` is
the operator that states it. It is not: `+0.0 == -0.0` is true, and a NaN is
unequal even to itself, so `==` is numeric equality. The claim now has its own
name, `bitIdentical()`, which compares the bits through `std::bit_cast`, and the
value types have no floating-point `==` at all — `-Wfloat-equal` would report
the one a defaulted `<=>` brings with it. A tolerance would still hide exactly
the drift being tested for; the collision is resolved by saying precisely what
is meant.
...
The last two are silenced at the site rather than in `.clang-tidy`, with the
reason written above it, and so is the one other lint suppression, on
`nearlyEqual`'s two genuinely interchangeable doubles. The compiler's warnings
follow the same rule. All of them are on, and the two switched off at a site
carry their reasons there: clang's report of `std::fputs` in each program's
`main`, which is the C library's interface, and gcc's report of clang's
`[[clang::lifetimebound]]`, an attribute gcc does not know. That placement is
the point: a suppression in the config file covers whatever comes along later,
while a suppression on the line covers only the line. Nothing in `.clang-tidy`
is disabled that the code could have satisfied instead — the enums carry an
explicit `std::uint8_t` base, the headers carry include guards, and the
redundant `{}` initializers are gone, because fixing those was cheaper than
