...
| Next task | [M1-08](plan/tasks/m1-08-solar-position.md), the solar position: ERFA's `eraEpv00` negated, geometric, 0.1″ and 1e-6 AU against the Horizons fixture, with a date outside its span reported by name. Its March-equinox test refers the declination to `intermediateFromInertial`, which M1-07 added |
| Before M1-08, still open | **Nothing recorded.** M1-08's own document carries no open question: its span behaviour was ruled on 2026-09-11 (decision 31), and its equinox test now has the TT-only rotation it needs. Its questions, if it raises any, go up before its code |
...
| Working branch | `master`. Everything through 2026-09-19 is pushed; the three commits of 2026-09-20 -- the `utcFromTai` fix, M1-86 and M1-07 -- are **local**, and nothing is pushed until the owner says so. `git fetch && git switch master` is all another machine needs. **Five merged branches can be deleted** whenever somebody feels like it, local and remote: `clang-23-2026-09`, `consistency-fixes-2026-09-13`, `docs-reorg`, `pre-docs-reorg-2026-09-09`, `review-fixes-2026-09` |
...
Totals: **1,180,234 assertions in 127 test cases**, and the same 1,180,234 under
...
skips one case and the total is 1,180,108. Catch2 **v3.16.0**.
...
only — with **zero warnings under `/Wall /WX /permissive-`**, and passes all 132
...
selects one and `ctest -N` lists **132**: the 127 Catch2 cases, the two
fixture tests, ERFA's two validation programs, and `orbsim_smoke`.
`ctest -LE gpu` lists 131, and `ctest -L fixtures` the two fixture tests.
