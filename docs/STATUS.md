...
| Component | State |
|---|---|
| `src/core/` | Scalars and vectors that carry their unit over mp-units, Quat, and `TimePoint` on five time scales. **UTC, TAI and TT now convert**, in exact integer picoseconds, over the IERS leap-second table in `core/LeapSeconds.hpp`; TDB and UT1 are M1-05. Complete for what exists. |
