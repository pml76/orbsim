...
Totals: **914,546 assertions in 104 test cases**, and the same 914,546 under
...
skips one case and the total is 914,420. Catch2 **v3.16.0**.
...
only — with **zero warnings under `/Wall /WX /permissive-`**, and passes all 109
...
selects one and `ctest -N` lists **109**: the 104 Catch2 cases, the two
fixture tests, ERFA's two validation programs, and `orbsim_smoke`.
`ctest -LE gpu` lists 108, and `ctest -L fixtures` the two fixture tests.
