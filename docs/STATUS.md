...
| Next task | [M1-09](plan/tasks/m1-09-orbsim-view.md), the `orbsim_view` library and `Mat4` -- the first render-side task, and the one that makes [ADR 0012](adr/0012-orbsim-view.md)'s Vulkan-free library the tests can link. It depends on M1-01 only, so nothing in phase A blocks it |
| Before M1-09, still open | **Nothing recorded.** Its questions, if it raises any, go up before its code |
...
| Working branch | `master`. Everything through 2026-09-20 is pushed, M1-86, M1-07 and M1-08 included -- the last two commits being the `JulianDate` lift and M1-08 itself, verified pushed rather than assumed. `git fetch && git switch master` is all another machine needs. **Five merged branches can be deleted** whenever somebody feels like it, local and remote: `clang-23-2026-09`, `consistency-fixes-2026-09-13`, `docs-reorg`, `pre-docs-reorg-2026-09-09`, `review-fixes-2026-09` |
...
| `tests/` | Nine Catch2 suites, 1,263,723 assertions in 135 test cases, plus ERFA's two validation programs, a GPU smoke test, two fixture tests and two libFuzzer targets. |
...
Totals: **1,263,723 assertions in 135 test cases**, and the same 1,263,723 under
...
and `20260920` in `tests/test_math.cpp` and `tests/test_earth_orientation.cpp`,
...
selects one and `ctest -N` lists **140**: the 135 Catch2 cases, the two
fixture tests, ERFA's two validation programs, and `orbsim_smoke`.
`ctest -LE gpu` lists 139, and `ctest -L fixtures` the two fixture tests.
Both Linux presets are core only and so run 139.
