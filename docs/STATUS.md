...
| Before M1-04, still open | **Four of the seven points are done (2026-09-17); three need a ruling.** **Done:** the `constexpr` work and its proofs -- `dotExact`, `crossExact`, `normSquaredExact` and `roundedToDouble` are `constexpr` with `static_assert`s proving the exactness the double-double conversion rests on, including that `dotExact({1e16,1,0},{1,1,0})` keeps in its low word the 1 a double loses; `isFinite(Vec3)`, `isFinite(StateVector)` and `elementsAreUsable` likewise, which needed a `constexpr isNaN` beside `isFinite` in `core/Scalar.hpp` because a parabola's `sma` is legitimately infinite; `orbit/Orbit.hpp` has its first `static_assert`s, proving every `OrbitError` describes itself and no two the same; and `Quantity::derived()` has its `[[nodiscard]]`. **Corrected while doing it:** the claim that six helpers were `constexpr`-able was wrong -- `factorOutScale` and `exactStateOf` use `std::abs`, `std::ilogb`, `std::scalbn` and `std::frexp`, none `constexpr` before C++26, the same trap `std::isfinite` sprang on MSVC. **Still open, and each is the owner's:** (a) the **Horizons terms**, question 8 of [`PROJECT_STATE.md`](PROJECT_STATE.md) section 7 -- the one with a date, since M1-06 runs after M1-04; (e) **`quickTwoSum`'s precondition is violated by `DoubleDouble::operator+`**, question 9, found by asserting it; (f) the two `assign*` out-parameters **stay**, question 10 -- converting them to return values pushes `elementsFromState` past `readability-function-size`, which is a better reason for their shape than the "defensible" this row used to claim |
...
| `tests/` | Four Catch2 suites, 642,205 assertions in 66 test cases, plus a GPU smoke test and a libFuzzer target. |
...
Totals: **642,205 assertions in 66 test cases**, and the same 642,205 under
...
selects one and `ctest -N` lists **67**: the sixty-six Catch2 cases plus
`orbsim_smoke`. `ctest -LE gpu` lists the sixty-six.
