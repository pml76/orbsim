...
| Next task | [M1-07](plan/tasks/m1-07-earth-orientation.md), precession, nutation and the Earth rotation angle: ERFA's `eraC2t06a`, `eraC2i06a` and `eraEra00`, the body-fixed frame and a TT-only rotation for M1-08 and M1-63, checked to **0.1 mas** over 1900-2100 against Skyfield 1.55 -- an equinox-based route where ERFA's is CIO-based. Its ten questions were ruled before its code: decisions 75-83 |
| Before M1-07, still open | **Nothing.** Both questions were answered on 2026-09-19, with eight more the task raised. The UT1 one (decision 66) became M1-86; the reference is Skyfield 1.55, whose independence and its limit -- a nutation ported from NOVAS, which shares IERS modules with SOFA -- are recorded in decision 75 |
...
Totals: **1,060,791 assertions in 112 test cases**, and the same 1,060,791 under
...
skips one case and the total is 1,060,665. Catch2 **v3.16.0**.
...
only — with **zero warnings under `/Wall /WX /permissive-`**, and passes all 117
...
selects one and `ctest -N` lists **117**: the 112 Catch2 cases, the two
fixture tests, ERFA's two validation programs, and `orbsim_smoke`.
`ctest -LE gpu` lists 116, and `ctest -L fixtures` the two fixture tests.
