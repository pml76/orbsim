...
| Before M1-04, still open | **Seven points, ruled on 2026-09-14 to be cleared before M1-04 starts.** They came out of the documentation-consistency sweep and are recorded here so they are not carried in a conversation. (a) **The JPL Horizons terms were read on 2026-09-17 and did not settle it**: no licence is stated anywhere and the SSD FAQ asks to be told what you intend to use and how. [`THIRD_PARTY.md`](../THIRD_PARTY.md) has the sources; it is now the owner's call, question 8 of [`PROJECT_STATE.md`](PROJECT_STATE.md) section 7, and the one open question with a date attached -- M1-06 commits the first fixture and runs immediately after M1-04. (b) Six exact-arithmetic helpers in `orbit/Orbit.cpp` -- `dotExact`, `crossExact`, `normSquaredExact`, `roundedToDouble`, `factorOutScale`, `exactStateOf` -- are `constexpr`-able and unmarked, so the exactness claims under them cannot be proved by `static_assert` the way the double-double layer beneath them is. (c) `isFinite(Vec3)`, `isFinite(StateVector)` and `elementsAreUsable` likewise, now nearly free since the `constexpr isFinite` moved into `core/Scalar.hpp`. (d) `orbit/Orbit.hpp` carries **zero** `static_assert`s, so its `constexpr describe()` is unproven -- against nine in `Time.hpp` and twenty-two in `DoubleDouble.hpp`. (e) `quickTwoSum`'s documented precondition `|a| >= |b|` is not asserted, which is exactly [`VERIFICATION.md`](VERIFICATION.md) rule 7's "only a bug can cause it"; it is on the hottest path and `DoubleDouble.hpp` does not include `Contract.hpp`, so it is not free. (f) `assignInPlaneAngles` and `assignConic` take an in/out `Elements&` -- the last out-parameters in the file since `stumpff` began returning a pair. (g) `Quantity::derived()` lacks `[[nodiscard]]`. The two **owner decisions** that belong with them are questions 6 and 7 of [`PROJECT_STATE.md`](PROJECT_STATE.md) section 7: whether `Vec3` stays unit-free, and whether MSVC should build at `/Wall` rather than `/W4` |
...
| `tests/` | Four Catch2 suites, 642,199 assertions in 65 test cases, plus a GPU smoke test and a libFuzzer target. |
...
selects one and `ctest -N` lists **66**: the sixty-five Catch2 cases plus
`orbsim_smoke`. `ctest -LE gpu` lists the sixty-five.
