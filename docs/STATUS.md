...
| `src/view/` | **New with M1-09.** The render-side maths Vulkan never touches, as `orbsim_view` -- which links `orbsim_core` and nothing else, and which `orbsim_core` cannot link back (a configure-time assertion says so). `Mat4<kInXyz, kInW, kOutXyz, kOutW>` and its `Vec4`, column-major, every block reference derived: `Transform` is metre-to-metre affine, `Projection` maps a world point to clip space where w is metres, and the divide lands in dimensionless normalised device coordinates. Header-only, so the library is INTERFACE until M1-11 adds `Camera.cpp`. |
...
| `tests/` | Ten Catch2 suites, 1,329,749 assertions in 144 test cases, plus ERFA's two validation programs, a GPU smoke test, two fixture tests and two libFuzzer targets. |
...
Totals: **1,329,749 assertions in 144 test cases**, and the same 1,329,749 under
...
selects one and `ctest -N` lists **149**: the 144 Catch2 cases, the two
fixture tests, ERFA's two validation programs, and `orbsim_smoke`.
`ctest -LE gpu` lists 148, and `ctest -L fixtures` the two fixture tests.
Both Linux presets are core only and so run 148, and so does a
...
themselves, so that browsing the directory finds it. **Twenty are accepted**:
...
proposed because the spike was built and measured first.
